...
SELECT
    ...
    COUNT(P.PhotoID) AS "Number of photos"
FROM
    Users U
LEFT JOIN
    Photos P ON U.UserID = P.UserID
GROUP BY
    U.Username
ORDER BY
...
-- Question 2
-- Find the total number of likes received for each photo.
-- Display the photo_id and the corresponding number of likes.
-- Order the result by the number of likes in descending order.

SELECT
    L.PhotoID,
    COUNT(L.UserID) AS "Number of likes"
FROM
    Likes L
GROUP BY
    L.PhotoID
...
-- Question 3
-- Identify the user who has posted the earliest photo.
-- Display their username and the date of the earliest photo.
-- Display the date in the format DD/MM/YYYY.

SELECT
    U.Username AS Username,
    TO_CHAR(MIN(P.PublicationDate), 'DD/MM/YYYY') AS "Earliest Photo Date"
FROM
    Users U
JOIN
    Photos P ON U.UserID = P.UserID
